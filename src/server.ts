import { extname, posix } from "https://deno.land/std@0.95.0/path/mod.ts";
import {
  HTTPSOptions,
  listenAndServe,
  listenAndServeTLS,
...
async function serveDir(
  serverArgs: FileServerArgs,
  req: ServerRequest,
  dirPath: string
): Promise<Response> {
  const showDotfiles = serverArgs.dotfiles ?? true;
  const dirUrl = `/${posix.relative(serverArgs.dirName, dirPath)}`;
...
export function serve(serverArgs: FileServerArgs): void {
  ...
  const dirName = serverArgs.dirName || "";
    ...
    try {
      ...
      if (fileInfo.isDirectory) {
        if (dirListingEnabled) {
          response = await serveDir(serverArgs, req, fsPath);
        } else {
